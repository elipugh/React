import os, random, operator, sys, csv
import numpy as np
...
def loadCsvData(fileName):
    ...
    with open(fileName) as f:
        ...
        for row in reader:
            ...
            doubleRow = []
            for value in row:
                examples.append((row[status_message].strip(), [int(row[num_loves]), int(row[num_wows]),\
                    int(row[num_hahas]), int(row[num_sads]), int(row[num_angrys])]))
    return examples
